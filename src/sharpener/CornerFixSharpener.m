...
// ---------------------------------------------------------------------------
// Lite-mode subclass swizzling: Chromium (Brave), Qt (AyuGram) and other apps
// use custom NSWindow subclasses that override _cornerMask, _effectiveCornerRadius,
// etc.  Swizzling on [NSWindow class] only affects windows that inherit those
// methods — subclass overrides are unaffected.  This function swizzles the
// corner methods on the ACTUAL runtime class of a window instance, using
// method_setImplementation to avoid corrupting NSWindow's method table.
// ---------------------------------------------------------------------------

static NSMutableSet<NSString *> *sCFXSwizzledSubclasses = nil;

static void CFXEnsureSwizzledSubclassSet(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sCFXSwizzledSubclasses = [NSMutableSet set];
    });
}

static void CFXSwizzleCornerMethodsOnClass(Class cls) {
    if (cls == nil || cls == [NSWindow class]) {
        return;
    }

    CFXEnsureSwizzledSubclassSet();
    NSString *className = NSStringFromClass(cls);
    @synchronized(sCFXSwizzledSubclasses) {
        if ([sCFXSwizzledSubclasses containsObject:className]) {
            return;
        }
        [sCFXSwizzledSubclasses addObject:className];
    }

    Class nsWindow = [NSWindow class];

    // For each corner method: if the subclass has its own IMP (different from
    // NSWindow's already-swizzled IMP), replace it with our replacement IMP.
    struct { SEL original; SEL replacement; } methods[] = {
        { NSSelectorFromString(@"_setCornerRadius:"),          @selector(cfx__setCornerRadius:) },
        { NSSelectorFromString(@"_setEffectiveCornerRadius:"), @selector(cfx__setEffectiveCornerRadius:) },
        { NSSelectorFromString(@"_effectiveCornerRadius"),     @selector(cfx__effectiveCornerRadius) },
        { NSSelectorFromString(@"_cornerRadius"),              @selector(cfx__cornerRadius) },
        { NSSelectorFromString(@"_topCornerRadius"),           @selector(cfx__topCornerRadius) },
        { NSSelectorFromString(@"_bottomCornerRadius"),        @selector(cfx__bottomCornerRadius) },
        { NSSelectorFromString(@"_cornerMask"),                @selector(cfx_cornerMask) },
        { NSSelectorFromString(@"_updateCornerMask"),          @selector(cfx__updateCornerMask) },
    };

    const int count = sizeof(methods) / sizeof(methods[0]);
    for (int i = 0; i < count; i++) {
        SEL origSel = methods[i].original;
        SEL replSel = methods[i].replacement;

        Method clsMethod = class_getInstanceMethod(cls, origSel);
        if (clsMethod == NULL) {
            continue;
        }

        // Check if this is inherited from NSWindow (already swizzled there)
        Method nsWindowMethod = class_getInstanceMethod(nsWindow, origSel);
        if (nsWindowMethod != NULL &&
            method_getImplementation(clsMethod) == method_getImplementation(nsWindowMethod)) {
            // Inherited — already swizzled on NSWindow, nothing to do
            continue;
        }

        // The subclass has its own implementation — replace it with ours.
        // Get our replacement IMP from the NSWindow category method.
        Method replMethod = class_getInstanceMethod(nsWindow, replSel);
        if (replMethod == NULL) {
            continue;
        }
        IMP replIMP = method_getImplementation(replMethod);
        method_setImplementation(clsMethod, replIMP);
        CFXLog(@"lite: swizzled %@ on class=%@", NSStringFromSelector(origSel), className);
    }
}
...
/// Lite mode: install only the pure getter/setter swizzles that return or
/// force radius=0.  No view-hierarchy walking, no notifications, no overlays.
/// Safe for Qt/Electron/Chromium apps whose NSWindow internals don't match AppKit assumptions.
- (void)installLiteHooks {
    CFXLog(@"installLiteHooks bundle=%@", _bundleIdentifier);
    Class nsWindow = [NSWindow class];

    // Setters — force 0.0
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_setCornerRadius:")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_setCornerRadius:"), @selector(cfx__setCornerRadius:));
    }
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_setEffectiveCornerRadius:")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_setEffectiveCornerRadius:"), @selector(cfx__setEffectiveCornerRadius:));
    }

    // Getters — return 0.0
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_effectiveCornerRadius")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_effectiveCornerRadius"), @selector(cfx__effectiveCornerRadius));
    }
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_cornerRadius")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_cornerRadius"), @selector(cfx__cornerRadius));
    }
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_topCornerRadius")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_topCornerRadius"), @selector(cfx__topCornerRadius));
    }
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_bottomCornerRadius")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_bottomCornerRadius"), @selector(cfx__bottomCornerRadius));
    }

    // _cornerMask — THIS is what the SkyLight compositor reads for the window silhouette.
    // Without this, the compositor keeps drawing rounded corners regardless of the radius getters.
    // Returns a square NSImage — no view hierarchy touching, safe for lite mode.
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_cornerMask")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_cornerMask"), @selector(cfx_cornerMask));
    }

    // _updateCornerMask — call original (updates layout) then force our radius.
    if (class_getInstanceMethod(nsWindow, NSSelectorFromString(@"_updateCornerMask")) != NULL) {
        (void)CFXSwizzleInstanceMethod(nsWindow, NSSelectorFromString(@"_updateCornerMask"), @selector(cfx__updateCornerMask));
    }

    // Hook window appearance to swizzle corner methods on the ACTUAL window subclass.
    // Chromium (Brave), Qt (AyuGram) etc. use custom NSWindow subclasses that override
    // _cornerMask / _effectiveCornerRadius — the NSWindow-level swizzles above don't
    // affect those overrides.  These hooks detect the real class at runtime and
    // method_setImplementation the subclass's own methods.
    CFXSwizzleInstanceMethod(nsWindow, @selector(makeKeyAndOrderFront:), @selector(cfx_lite_makeKeyAndOrderFront:));
    CFXSwizzleInstanceMethod(nsWindow, @selector(orderFront:), @selector(cfx_lite_orderFront:));
    CFXSwizzleInstanceMethod(nsWindow, @selector(orderFrontRegardless), @selector(cfx_lite_orderFrontRegardless));

    // Swizzle and apply radius to any windows that already exist
    for (NSWindow *window in NSApp.windows) {
        CFXSwizzleCornerMethodsOnClass([window class]);
        @try {
            [(id)window setValue:@(0.0) forKey:@"cornerRadius"];
        } @catch (NSException *e) { (void)e; }
        @try {
            for (NSString *selName in CFXCornerSelectors()) {
                (void)CFXInvokeCGFloatSetter(window, selName, 0.0);
            }
            [window invalidateShadow];
        } @catch (NSException *e) {
            CFXLog(@"lite: existing window apply failed: %@", e);
        }
    }
}
...
- (void)updateShadowForWindow:(NSWindow *)window radius:(CGFloat)radius {
    ...
    // Keep the shadow enabled even at radius==0. On macOS 26 (Tahoe) with
    // _setEffectiveCornerRadius:0 working correctly, the shadow renders with
    // square edges. Disabling it removes the visual window boundary and can
    // cause a blank strip artifact when zooming. Opt out via CFX_NO_SHADOW=1.
    BOOL suppressShadow = [NSProcessInfo.processInfo.environment[@"CFX_NO_SHADOW"] boolValue];
    window.hasShadow = suppressShadow ? NO : YES;
}
...
- (BOOL)shouldReplaceSystemCornerMaskForWindow:(NSWindow *)window {
    ...
    // In lite mode, be permissive — Qt/Chromium windows may not pass the strict
    // NSWindowStyleMaskTitled / size / class filters, but the _cornerMask swizzle
    // only returns an NSImage, so it's safe to apply broadly.
    if (sCFXLiteMode) {
        return YES;
    }
...
- (void)updateOverlayForWindow:(NSWindow *)window radius:(CGFloat)radius {
    ...
    // The overlay draws colored caps in the corners to mask residual rounding.
    // On macOS 26 (Tahoe), _setEffectiveCornerRadius:0 works correctly and the
    // overlay only creates visual artifacts (gray strips along the top edge).
    // Disable the overlay by default; opt in via CFX_OVERLAY=1 for systems that need it.
    BOOL overlayEnabled = [NSProcessInfo.processInfo.environment[@"CFX_OVERLAY"] boolValue];

    if (!overlayEnabled || radius < 0.0) {
        if (overlay != nil) {
...
// --- Lite-mode hooks: detect window subclass, swizzle corner methods, and apply radius ---

- (void)cfx_lite_applyCornerRadius {
    CFXSwizzleCornerMethodsOnClass([self class]);
    // Apply private setters directly on the window object — bypass CFXIsStandardAppWindow
    // filter (Qt/Chromium windows don't pass it).  No view hierarchy walking — safe.
    @try {
        // KVC — may or may not work on custom subclasses, but harmless to try
        [(id)self setValue:@(0.0) forKey:@"cornerRadius"];
    } @catch (NSException *e) { (void)e; }
    @try {
        // Private setters — these are what actually drive the compositor
        for (NSString *selName in CFXCornerSelectors()) {
            (void)CFXInvokeCGFloatSetter(self, selName, 0.0);
        }
        [self invalidateShadow];
    } @catch (NSException *e) {
        CFXLog(@"lite: applyCornerRadius failed: %@", e);
    }
}

- (void)cfx_lite_makeKeyAndOrderFront:(id)sender {
    [self cfx_lite_makeKeyAndOrderFront:sender];
    [self cfx_lite_applyCornerRadius];
}

- (void)cfx_lite_orderFront:(id)sender {
    [self cfx_lite_orderFront:sender];
    [self cfx_lite_applyCornerRadius];
}

- (void)cfx_lite_orderFrontRegardless {
    [self cfx_lite_orderFrontRegardless];
    [self cfx_lite_applyCornerRadius];
}
...
- (void)cfx__updateCornerMask {
    ...
    if ([sharpener shouldReplaceSystemCornerMaskForWindow:self]) {
        // Call the original first so macOS updates its internal window layout/margins,
        // then override the corner radius. Suppressing _updateCornerMask entirely
        // causes a blank strip at the top when the window is zoomed/resized.
...
// ---------------------------------------------------------------------------
// Lite mode: for apps with non-standard window chrome (Qt, Electron, custom
// NSWindow subclasses) where walking the view/layer hierarchy crashes.
// In lite mode we install ONLY the getter/setter swizzles — the compositor
// reads _effectiveCornerRadius and gets 0.0, giving square corners without
// touching the view hierarchy.
// ---------------------------------------------------------------------------
static NSArray<NSString *> *CFXLiteModeBundleIDs(void) {
    return @[
...
static BOOL CFXIsLiteModeBundle(NSString *bundleIdentifier) {
